...
Compile 30–60 public German-language far-right/conspiracy-milieu channels from **published research only**: CeMAS publications (e.g. their Telegram monitoring reports), ISD Germany reports, Amadeu Antonio Stiftung monitoring. For each channel write one CSV row; `source_list` names the exact publication (title + year), `inclusion_criterion` quotes or paraphrases the report's categorisation (e.g. `"CeMAS 2022 'Telegram-Radikalisierung' — Q-adjacent conspiracy channel"`). Do NOT invent channels; if a report names fewer, a shorter list is correct. **Bare `curl` to `t.me` is forbidden — it is a network path with no ProtonVPN gate at all and can leak the user's home IP to Telegram.** Verify each username still resolves with the gated reachability checker instead: `python3 -m tools.social.telegram.check_reachable <username> [<username> ...]` (or `--db PATH` to check every already-tracked channel). It hard-aborts with exit 2 if ProtonVPN is not active, and every check goes through `preview.fetch_page`, which enforces the same gate again at the network boundary. Note dead ones with `inclusion_criterion` suffix `" (unreachable 2026-08)"` — keep the row; churn is data. All rows enter as `seed_pending`; **the user approves via channels.py — the implementer never approves channels.**
