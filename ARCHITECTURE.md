...
| `ModelConfig` | An LLM endpoint + model id + prompt. The Annotation Queue's model picker enumerates these. Presets seeded on first launch: Gemma 3 4B (default), Qwen 3 8B / Qwen 3.6 27B / Qwen 2.5 7B (Ollama), Gemma 4 26B (Ollama), Phi 4 14B (Ollama), Apple Foundation Models (non-default — blocked by safety guardrails on hate content). The `endpoint` field doubles as a transport tag: `"apple-foundation"` → on-device Apple model, any URL → Ollama. See `Research/LLM_Hate_Counter_Speech_Classification_from_CC.md` for the model comparison. |
...
- **`AppleFoundationModelClient`** — Apple's on-device Foundation Models framework (`SystemLanguageModel`, `LanguageModelSession`). ~3 B parameters, ~2 GB resident memory, runs on the Neural Engine; macOS 26+. Uses the `@Generable` macro for structured output. **NOT the default**: Apple's guardrails refuse hate-content classification (even under `.permissiveContentTransformations`) — the framework returns `.guardrailViolation` for nearly every Bluesky reply containing slurs, threats, or political invective. The client is kept for adjacent analyses (translation, summary, topic classification) where the inputs don't trigger the safety filter.
